...
import { Package, Plus, ArrowRight, ArrowLeftRight } from 'lucide-react';
        ...
        {isRelationship && hasRelationshipData ? (
          <div className="space-y-2">
            <div className="flex items-center gap-3 min-w-0">
              <span className="text-lg flex-shrink-0">{renderIcon(object.icon)}</span>
              <div className="flex-1 min-w-0">
                <div className="font-medium truncate">
                  {searchQuery.trim() && object.name ? renderHighlightedText(object.name, searchQuery.trim()) : object.name}
                </div>
              </div>
            </div>
...
const ObjectsPage = () => {
  ...
  const [objects, setObjects] = useState<(Object & { subjectObject?: Object; objectObject?: Object })[]>([]);
  ...
  function updateFilters(newQ: string, newIsEvent: boolean, newIsPerson: boolean, newIsRelationship: boolean, newIsPromise: boolean) {
    ...
    const fetchObjects = async () => {
      ...
      try {
        ...
        const pipeline: unknown[] = [
          ...
          ...(q.trim() ? [{ $sort: { score: { $meta: "textScore" }, _id: 1 } }] : [{ $sort: { _id: 1 } }]),
          { $limit: 100 }
        ];
        ...
        <div className="flex flex-wrap gap-6">
          <div className="flex items-center space-x-2">
            <Checkbox
              id="isEvent"
              checked={isEvent}
              onCheckedChange={(checked) => updateFilters(q, checked === true, isPerson, isRelationship, isPromise)}
            />
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              id="isPerson"
              checked={isPerson}
              onCheckedChange={(checked) => updateFilters(q, isEvent, checked === true, isRelationship, isPromise)}
            />
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              onCheckedChange={(checked) => updateFilters(q, isEvent, isPerson, checked === true, isPromise)}
            />
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              id="isPromise"
              checked={isPromise}
              onCheckedChange={(checked) => updateFilters(q, isEvent, isPerson, isRelationship, checked === true)}
            />
      ...
      {objects.length === 0 ? (
        <Card className="p-8 text-center">
          ...
          <p className="text-muted-foreground">
            No objects found{q.trim() || isEvent || isPerson || isRelationship || isPromise ? ' matching your filters' : ''}. Objects are created through the application.
          </p>
        </Card>
      ) : (

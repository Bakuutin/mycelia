...
export function MetadataDisplay({ object }: MetadataDisplayProps) {
  ...
  const entityType = object?.metadata?.entityType;
      ...
      <Card className="p-4 bg-muted/50">
        ...
        <dl className="space-y-3 text-sm">
          <div className="flex justify-between items-center">
            ...
            <dd className="text-foreground">{formatTime(new Date(object.createdAt))}</dd>
          </div>
          <div className="flex justify-between items-center">
            <dt className="text-muted-foreground">Updated:</dt>
            <dd className="text-foreground">{formatTime(new Date(object.updatedAt))}</dd>
          </div>
          {entityType && (
            <div className="flex justify-between items-center">
        ...
        <Card className="p-4 bg-muted/50">
          ...
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between items-center">
              ...
              <dd className="font-mono text-foreground">{extractedWith.model}</dd>
            </div>
            <div className="flex justify-between items-center">
              <dt className="text-muted-foreground">Extracted:</dt>
              <dd className="text-foreground">{formatTime(new Date(extractedWith.timestamp))}</dd>
            </div>
          </dl>
        </Card>
      )}

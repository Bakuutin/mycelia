...
export interface SearchableSelectMultipleProps extends
  Omit<
    ...
    const getAllOptions = React.useCallback((): MultiSelectOption[] => {
      ...
      if (import.meta.env.DEV && duplicates.length > 0) {
        const action = deduplicateOptions
          ...
          : "detected";
        console.warn(
          `SearchableSelectMultiple: Duplicate option values ${action}: ${
            duplicates.join(
              ", ",
            )
          }. ` +
        ...
        if (diff > 0) {
          ...
          if (addedLabels.length === 1) {
            announce(
              `${
                addedLabels[0]
              } selected. ${selectedCount} of ${totalOptions} options selected.`,
            );
          } else {
            announce(

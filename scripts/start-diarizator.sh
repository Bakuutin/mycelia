...
if [[ -z "${HF_TOKEN:-}" ]]; then
  for env_file in "${repo_root}/.env" "${repo_root}/diarizator/.env"; do
...
echo "Reachable from Mycelia: http://diarizator:${port}"
echo "First start downloads ~1.5 GB of models; wait for 'Models ready'."
echo

exec docker compose --profile diarization up -d --build diarizator

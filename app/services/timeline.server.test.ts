...
function setup(policies?: Policy[]) {
  auth = new Auth({
    principal: "timeline-tester",
    policies: policies ?? [
      { resource: "db/*", action: "*", effect: "allow" },
    ],
  });
  const mongoResource = new MongoResource();
  auth.resourceManager.registerResource(mongoResource);

  mongoResource.getRootDB = async () => ({
    collection: fn(() => ({
      find: fn(() => []),
      findOne: fn(() => null),
      aggregate: fn(() => ({ toArray: fn(() => []) })),
      bulkWrite: fn(() => ({ ok: 1 })),
      createIndex: fn(() => ({ ok: 1 })),
      indexes: fn(() => []),
    })),
...
Deno.test("fetchTimelineData should return correct structure", async () => {
  setup();
  ...
  (auth.resourceManager.resources.get("tech.mycelia.mongo") as any)
    .getRootDB = async () => ({
      collection: fn((name: string) => {
        if (name.startsWith("histogram_")) {
          return {
            find: fn(() => mockHistogramData),
          };
        } else if (name === "transcriptions") {
          return {
...
Deno.test("fetchTimelineData should sort transcripts by start time", async () => {
  setup();
...
Deno.test("fetchTimelineData should handle transcript segments correctly", async () => {
  setup();

...
const defaultPolicies: Policy[] = [{ resource: "**", action: "*", effect: "allow" }];
...
export default function Layout() {
  const { auth, keys } = useLoaderData<{ auth: Auth; keys: Array<{ _id: { $oid: string } | string; name: string; createdAt: { $date: string }; isActive: boolean; openPrefix: string; policies: Policy[] }> }>();
  const actionData = useActionData<{ created?: string; revoked?: boolean; updated?: boolean; error?: string }>();
            ...
            <div className="flex flex-col gap-4">
              <Form method="post" className="flex flex-col gap-3">
                ...
                <div className="grid gap-2">
                  ...
                  <Input id="name" name="name" placeholder="e.g. cli, dev, integration" />
                </div>
                <div className="grid gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {actionData?.error && (
                    <span className="text-xs text-red-500">{actionData.error}</span>
                  )}
                </div>
              </Form>
          ...
          <CardContent>
            {keys.length === 0 ? (
              ...
              <ul className="divide-y rounded border">
                {keys.map((k, idx) => {
                  const id = typeof k._id === "string" ? k._id : (k._id as any)?.$oid ?? "";
                  const created = new Date(k.createdAt.$date).toLocaleString();
                  return (
                    <li key={id} className="flex flex-col gap-3 p-3">
                      ...
                      {k.isActive && (
                        <Form method="post" className="grid gap-2 rounded border bg-muted/40 p-3">
                          <input type="hidden" name="intent" value="update" />
                          <input type="hidden" name="id" value={id} />
                          <div className="grid gap-2">
                            <Label htmlFor={`name-${id}`}>Name</Label>
                            <Input id={`name-${id}`} name="name" defaultValue={k.name} />
                          </div>
                          <div className="grid gap-2">
                            <Label htmlFor={`policies-${id}`}>Policies JSON</Label>
                            <textarea
                              id={`policies-${id}`}
                              name="policies"
                              defaultValue={JSON.stringify(k.policies ?? defaultPolicies, null, 2)}
                              className="min-h-28 w-full rounded-md border bg-background p-2 font-mono text-xs"
                            />
                          </div>
                          <div className="flex items-center gap-2">

...
export async function loader({ params, request }: LoaderFunctionArgs) {
  ...
  const files = await fs({ action: "find", bucket: uploadBucketName, query: { _id: new ObjectId(id) } });
  if (!files || files.length === 0) {
    return new Response("Not found", { status: 404 });
  }
  const file = files[0];
  const data: Uint8Array = await fs({ action: "download", bucket: uploadBucketName, id });
  const ext: string = file?.metadata?.extension || (file?.filename?.split(".").pop() ?? "");

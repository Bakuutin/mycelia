...
Deno.test(
  ...
  withFixtures([
    ...
    try {
      ...
      // --- 3. Test transcription_sequence_closer trigger ---
      // This is also triggered by audio_chunks
      const closerJob = await db.collection("jobs").findOne({ type: "transcription_sequence_closer" });
      expect(closerJob).not.toBeNull();
      console.log("[Test] Sequence closer job successfully triggered");

...
function useLaneLayout(
  items: ReturnType<typeof useEvents>["items"],
  xFor: (d: Date) => number,
) {
  return useMemo(() => {
    const placed: Array<
      { event: EventItem; startX: number; endX: number; lane: number }
    > = [];
    ...
    for (const category of categories) {
      ...
      for (const ev of sorted) {
        ...
        const endX = xFor(
          ev.end ?? (ev.kind === "range" ? new Date() : ev.start),
        );
...
export const EventComponent: React.FC<
  { p: any; topMargin: number; laneHeight: number; event: EventItem }
> = ({ p, topMargin, laneHeight, event }) => {
...
const eventFormSchema = z.object({
  ...
  start: z.string().min(1, "Start date is required").transform((str) =>
    new Date(str)
  ),
...
const EventForm: React.FC<EventFormProps> = (
  { defaultValues, onSubmit, onDelete, submitLabel, isEdit },
) => {
  const {
    register,
    handleSubmit,
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-4 items-center gap-4">
          ...
          <div className="col-span-3">
            <Input
              id="title"
              placeholder="E.g. Started a new job"
              {...register("title")}
        ...
        <div className="grid grid-cols-4 items-center gap-4">
          ...
          <div className="col-span-3">
            <Input
              ...
              placeholder="E.g. work, life, education"
              {...register("category")}
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-4 items-center gap-4">
          ...
          <div className="col-span-3">
            ...
            />
            {errors.start && (
              <p id="start-error" className="text-xs text-red-500 mt-1">
...
const EditEventSheet: React.FC = () => {
  ...
  const defaultValues: Partial<EventFormInput> = useMemo(() => {
    ...
    return {
      ...
      end: editingEvent.kind === "range" && editingEvent.end
        ? toLocalDateTime(editingEvent.end)
        : "",
    };
  }, [editingEvent]);

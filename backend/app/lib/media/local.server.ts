...
function findCapturedAt(metadata: Record<string, unknown>): Date | undefined {
  ...
  const format = metadata.format && typeof metadata.format === "object"
    ...
    : {};
  const tagObjects = [format.tags, ...streams.map((s: any) => s?.tags)]
...
export async function inspectLocalMedia(
  ...
  const data = await Deno.readFile(resolved.realPath);
  const detected = detectMime(data.slice(0, 16));
  ...
  if (detected.kind === "pdf") {
    ...
    metadata = await ffprobe(resolved.realPath);

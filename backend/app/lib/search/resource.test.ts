...
Deno.test(
  "deepResearch searches across all sources",
  withFixtures(["Admin", "Mongo"], async (admin: Auth, { db }) => {
    const resource = await getSearchResource(admin, db);

    await insertTranscription(db, { text: "Discussed project timeline" });
    await insertMessage(db, { text: "Project update needed" });
    await insertObject(db, { name: "Project Alpha", isEvent: true });
...
Deno.test(
  "deepResearch can exclude sources",
  withFixtures(["Admin", "Mongo"], async (admin: Auth, { db }) => {
    const resource = await getSearchResource(admin, db);

    await insertTranscription(db, { text: "Therapy session notes" });
    await insertMessage(db, { text: "Therapy appointment" });
    await insertObject(db, { name: "Therapist", isPerson: true });
    ...
    expect(result.results[0].source).toBe("transcription");
  }),
);

Deno.test(
  "deepResearch respects date filters",
  withFixtures(["Admin", "Mongo"], async (admin: Auth, { db }) => {
    const resource = await getSearchResource(admin, db);
    ...
    await insertTranscription(db, { text: "Recent work discussion", start: recentDate, end: recentDate });
    await insertTranscription(db, { text: "Old work discussion", start: oldDate, end: oldDate });

    const result = await resource({
      action: "deepResearch",
      query: "work",
      startDate: "30d" as unknown as Date,
      includeTranscriptions: true,
      includeMessages: true,
      includeObjects: true,
      limit: 30,
    ...
    expect(result.totalResults).toBe(1);
    expect(result.results[0].content).toBe("Recent work discussion");
  }),
);

Deno.test(
  "deepResearch returns empty results gracefully",
  withFixtures(["Admin", "Mongo"], async (admin: Auth, { db }) => {
    const resource = await getSearchResource(admin, db);

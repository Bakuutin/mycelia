...
function getHistogramCollections(): string[] {
  return [ "5min", "1hour", "1day", "1week"].map((resolution: string) =>
...
async function ensureMessageIndexes(db: Db): Promise<void> {
    await ensureIndexExists(
      db,
      "messages",
...
export async function up(db: Db, client: MongoClient): Promise<void> {
  console.log("Running init migration: Ensuring all MongoDB collections exist...");
...
export async function down(db: Db, client: MongoClient): Promise<void> {
    console.log("Downgrade from init not implemented to avoid data loss");
}

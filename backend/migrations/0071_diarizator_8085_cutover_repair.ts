...
const PREVIOUS_CUTOVER_AT = new Date("2026-08-23T21:47:00.000Z");
...
const REPAIR_SOURCE = "historical_route_inference_0071";

const reused8085Query = {
  type: { $in: ["diarization", "enrollment", "profileReenrollment"] },
  createdAt: {
    $gte: CURRENT_8085_CUTOVER_AT,
    $lt: PREVIOUS_CUTOVER_AT,
  },
  $or: [
  ...
  await db.collection("jobs").updateMany(
          ...
          $or: [
            {
              ...
              "routingContext.embeddingSpaceId": LEGACY_GPU_EMBEDDING_SPACE_ID,
            },
            {
    ...
    },
    {
      $set: {
        ...
        "routingContext.runtimeProvenanceSource": REPAIR_SOURCE,
        ...
        "data.routingContext.runtimeProvenanceSource": REPAIR_SOURCE,
      },
    },
  );
}
  ...
  await db.collection("jobs").updateMany(
    ...
    },
    {
      $set: {
        "routingContext.embeddingSpaceId": LEGACY_GPU_EMBEDDING_SPACE_ID,
        "routingContext.runtimeProvenanceSource": INCORRECT_SOURCE,
        "data.routingContext.embeddingSpaceId": LEGACY_GPU_EMBEDDING_SPACE_ID,
        "data.routingContext.runtimeProvenanceSource": INCORRECT_SOURCE,
      },
    },
  );

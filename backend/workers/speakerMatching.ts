...
export const schema = z.object({
  ...
  limit: z.number().int().positive().default(10000)
    .describe(
      "Maximum number of diarization segments to process in this job run",
    ),
  ...
  threshold: z.number().min(0).max(1).default(0.35)
    .describe(
      "Minimum cosine similarity (0-1) to accept a speaker match. Higher = stricter matching",
    ),
  /** Only match for specific profile ID (for re-matching) */
  profile_id: z.string().optional()
    .describe(
      "Optional: Only match segments against this specific speaker profile ID (for targeted re-matching)",
    ),
